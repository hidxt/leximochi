...
/** 我的：账号信息与登录设备管理 */
export function MePage({ api, session, me, onLoggedOut }: MePageProps): ReactElement {
    ...
    <>
      <section className="card">
        ...
        <dl className="meta">
          ...
          <dt>恢复码</dt>
          <dd>注册时生成的一组一次性恢复码，请离线保存好；忘记密码时用它重置。</dd>
        </dl>
      ...
      <section className="card">
        ...
        <p className="subtitle">发现不认识的设备时，删除该会话即可让它立即失效。删除后对方需重新登录。</p>
        ...
        {sessions === null ? (
          ...
          <>
              ...
              <div className="session" key={item.id}>
                <div>
                  <div>
                    ...
                    {item.isCurrent ? (
                      <span className="seal" style={{ marginLeft: 8 }}>
                        当前
                      </span>
                    ) : null}
                  </div>
            ...
            {others.length === 0 ? <p className="field__hint">目前只有这一台设备登录。</p> : null}
          </>
        )}
      </section>
    </>
