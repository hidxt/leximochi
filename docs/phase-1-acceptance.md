...
**已知取舍（待用户确认）**：RN 模板默认提交 `android/app/debug.keystore`（公开的调试密钥，口令固定为 `android`，仅用于 debug 签名），但本项目红线禁止提交任何 `*.keystore`，因此该文件**未入库**，并在 `apps/mobile/.gitignore` 注释中写明 `keytool` 生成命令以保证新克隆可复现构建。如用户希望沿用 RN 约定提交该公开调试密钥，需在安全红线中登记为显式例外后再改。
