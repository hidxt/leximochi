...
| --- | --- | --- | --- |
| GET | `/health` | 公开 | 存活检查 |
| GET | `/wordbooks` | 需登录 | 词库列表（数据驱动，新增词库无需改代码） |
| GET | `/wordbooks/:key/version` | 需登录 | 词库版本与词数（供离线下载比对） |
| GET | `/wordbooks/:key/words` | 需登录 | 分页导出完整词条（离线学习），limit ≤ 200 |
| GET | `/words/search` | 需登录 | 按词形搜索（LIKE 通配符已转义） |
...
| GET | `/review/history` | 需登录 | 复习历史分页（答题时间倒序，游标不透明） |
| GET | `/review/stats` | 需登录 | 学习统计：今日量/正确率/平均用时/状态分布/近 7 日趋势 |
...
| `apps/mobile` | ✅ 可构建：`npm run build:android -w @leximochi/mobile` → `apps/mobile/android/app/build/outputs/apk/debug/app-debug.apk`（Phase 2 复测：`BUILD SUCCESSFUL in 6m 42s`，约 124 MB，含 4 ABI 与 AsyncStorage 原生模块）。**未做运行验证**（本机无 AVD / 无真机）；离线词库的下载/读取逻辑有 7 个单测覆盖 |
